...
// 循环生成面板li
function getAreaList(json){
	var area = $("#area");	
	if (json) {
		for (var i=0; i < json.length; i++) {	
	 	 area.append("<li class='longer-area'><a href='#none' data-value='"+json[i].id+"' >"+json[i].name+"</a></li>");
		};
...
function setTableTitle(){
	var title = $("#select");
	title.empty();
	for (var i=0; i < areaId.length-1; i++) {
	  title.append("<a href='#none' data-value='"+areaId[i]+"' data-level='"+i+"'>"+areaTitle[i]+"</a><i class='fa fa-angle-right'></i>");
	};
	//题目最后一个，不再增加箭头
	title.append("<a href='#none' data-value='"+areaId[i]+"' data-level='"+i+"'>"+areaTitle[i]+"</a>");	
}
// 设置当前辖区后——标题
function setTitle(){
	$("#title i").html(areaTitle[areaTitle.length-1]);
}
var areaTitle = [];	//当前地区name
var areaId = [];	//当前地区id
var arrIndex = 1;  //json数组访问下标,当前level的下一级

//  $(this).attr("data-value")为点中的id
$(function(){
	getAreaList(table0Json[1]);			
	$("#area").on("click","a",function(){	
		if (arrIndex<json.length) {// 将此if全部注释掉，点击北京，加载下一级，当前则是点击北京加载北京同级	
			$("#area").empty();	//清空area			
			areaTitle.push($(this).text());//将点击字符串传入areaTitle
			areaId.push($(this).attr("data-value"));	
			getAreaList(json[arrIndex][$(this).attr("data-value")+""]);// 通过  $(this).attr("data-value")获取字符串	
			if (arrIndex ==1) {//初次点击面板，设置title
				$("#title i").html($(this).text());
			};
			arrIndex++;
			setTableTitle();				
		}else{//点击到最后一层，不再加载列表
	...
	$("#select").on("click","a",function(){		//点击标题栏某个	id=select	
		...
		if (level == 0) {//如果点击的是title第一级
			areaTitle.splice(0,areaTitle.length);
			areaId.splice(0,areaId.length);
			$(this).nextAll().remove();	
			$("#area").empty();	//清空area	
			getAreaList(json[level][1]);	
			arrIndex = level+1;				
		}else{
			areaTitle = areaTitle.splice(0,level+1);
			areaId    = areaId.splice(0,level+1);	
			$(this).nextAll().remove();	
			$("#area").empty();	//清空area			
			getAreaList(json[level+1][$(this).attr("data-value")+""]);// 通过  $(this).attr("data-value")获取字符串
			arrIndex = level+2;  
		}				
	});
});
